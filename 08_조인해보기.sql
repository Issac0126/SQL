...
테이블 이름을 생략해도 무방하다. 
그러나 해석의 명확성을 위해 테이블 이름에 소속을 표현해주는 것을 지향한다.

테이블 이름이 너무 길 시에는 ALIAAS를 작성하여 칭한다.
두 테이블 모두 가지고 있는 컬럼의 경우에는 반드시 소속을 명시해주어야 한다.
*/
...
3. 위의 결과와 동일한 departments_id 를 가진 
...
WHERE e.department_id = d.department_id(+) --OUTER 진행
AND d.location_id = loc.location_id; --INNER 진행. (여기가 우선.)
/*
employees 테이블에는 존재하고, departments 테이블에는 존재하지 않아도
(+)가 붙지 않은 테이블을 기준으로 하여 departments 테이블이 
조인에 참여하라는 의미를 부여하기 위해 기호를 붙인다.
