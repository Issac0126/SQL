...
-- 양쪽 테이블에 모두 존재하기 때문.
-- 이럴 때는, 컬럼에 테이블 이름을 붙이던지
-- 별칭을 써서 확실하게 지목을 해야한다.
SELECT
...
SELECT
    *
FROM info i 
LEFT JOIN auth a --i를 기준으로 a를 붙인다.
ON i.auth_id = a.auth_id;


-- 좌측 테이블과 우측 테이블 데이터를 모두 읽어
-- 중복된 데이터는 삭제되는 외부 조인. 
SELECT *
...
-- CROSS JOIN은 JOIN 조건을 설정하지 않기 떄문에
...
- info, auth 테이블 사용
- job 컬럼이 scientist인 사람의 id, title, content, job
*/
SELECT
