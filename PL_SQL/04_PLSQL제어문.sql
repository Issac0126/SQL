...
BEGIN
    IF v_num1 > v_num2
    THEN dbms_output.put_line(v_num1||'이(가) 큰 수입니다.');
    ELSE dbms_output.put_line(v_num2||'이(가) 큰 수입니다.');
    END IF;
END;
...
BEGIN
    ...
    --부서는 10,20,30등이기 때문에 -1만큼 반올림. (ex/ 12->10)
    ...
    IF v_salary <= 5000 
        ...
        ELSIF v_salary <= 9000 THEN dbms_output.put_line('중간');
