...
EXEC my_new_job_proc('JOB4', 'test job1', 10000, 20000);
...
CREATE OR REPLACE PROCEDURE my_new_job_proc  --기존 프로시저 구조를 수정. 
    (p_job_id IN jobs.job_id%TYPE, --매개변수
    p_job_title IN jobs.job_title%TYPE,
    p_min_sal IN jobs.min_salary%TYPE := 0 ,
    p_max_sal IN jobs.max_salary%TYPE :=1000,
...
BEGIN
    my_new_job_proc('JOB4', 'test_job1', 2000, 8000, str);
...
-- IN, OUT을 동시에 처리
CREATE OR REPLACE PROCEDURE my_parameter_test_proc22
    (p_var1 IN VARCHAR2, 
    p_var2 OUT VARCHAR2, --OUT 변수는 프로시저가 끝나기 전까지 값의 할당이 안됨.
    p_var3 IN OUT VARCHAR2 --IN, OUT 둘 다 가능함.
    )
IS
...
--    p_var1 := '결과1'; IN 변수는 값 할당 불가. (OUT으로 쓸 수 없음.)
    p_var2 := '결과2';
    p_var3 := '결과3';
END;
...
BEGIN
    mt_parameter_test_proc(v_var1, v_var2, v_var3);
